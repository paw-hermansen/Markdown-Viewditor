...
- Prepare test files:
  - A `.md` file with rich content (headings, code, images, tables, frontmatter, footnotes)
  - An empty `.md` file
  - A large `.md` file (5000+ lines)
  - A `.md` file with YAML frontmatter (standard key-value)
  - A `.md` file with skill-style frontmatter (`name` + `description`)
  - A read-only `.md` file
  - A `.md` file with CRLF line endings
  - A UTF-8 BOM `.md` file
...
| 5.2 | Italic | `I` | Wraps in `*...*`, viewer shows *italic* |
...
## Appendix A: Suggested Test Markdown File

Create a file named `test-all-features.md` with the following content to use as the primary test document:
...
<img src="./test-image.png" alt="HTML img tag" width="200">
