...
| 13.2 | Dialog appearance (neutral / ODT) | With confirmation ON, click "Export as ODT"                | Title: "Export" / "Export / Print"; message: "This export always uses a neutral, printer-friendly style."; no theme line; shows ODT options (math rasterize, "SVG images & Mermaid diagrams" rasterize, resolution) |
...
| 13.56a | Mermaid — vector (default)        | Export a document with a ` ```mermaid ` fence (rasterize off) | Diagram embedded as `Pictures/*.svg` (not source code)                             |
| 13.56b | Mermaid — rasterized PNG          | Tick "SVG images & Mermaid diagrams" → "Rasterize as PNG images"; export | Diagram replaced with a PNG frame                                       |
| 13.56c | Mermaid — render failure fallback | Export an invalid ` ```mermaid ` fence                     | Source kept as a preformatted code block                                             |
